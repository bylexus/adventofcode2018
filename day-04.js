 ...
 * Data Structure:
 * Object {
 *      guard-id: [array of 60 (minutes)]
 * }
 * Each guard has an array with 60 minutes: Each minute place is initialized with 0, then all guards
 * are entered in the arrays, increasing the minutes where they sleep by 1.
 * In the end, there is a minute array, containing 1+ entries for all guards.
 *
 */

let schedule = {};
let parser = new RegExp('\\[([0-9-]+)\\s+(\\d+):(\\d+)\\].*((\\#(\\d+))|(wakes)|(asleep)).*');
let lastGuardId = null;
let asleepStart = null;
let asleepEnd = null;
input.forEach(line => {
    // Step 1: Parse input
    let match = line.match(parser);
    if (match) {
        lastGuardId = match[6] || lastGuardId;

        let info = {
            date: match[1],
            hour: match[2],
            minute: match[3],
            wakeup: match[4] === 'wakes',
            asleep: match[4] === 'asleep',
            begin: match[6] !== undefined,
            guardId: lastGuardId
        };

        if (info.asleep) {
            asleepStart = info.minute;
        }
        if (info.wakeup) {
            asleepEnd = info.minute;
            if (!info.guardId || asleepStart > asleepEnd) {
                console.err(info);
                throw new Error("Failed! Someting went wrong!");
            }

            // Fill in the guard's sleep schedule:
            if (!schedule[info.guardId]) {
                schedule[info.guardId] = new Array(60);
            }
            for (let i = asleepStart; i < asleepEnd; i++) {
                schedule[info.guardId][i] = (schedule[info.guardId][i]||0)+1;
            }
        }
    }
});

// Step 3: Now find the guard with the most sleep minutes:
let maxMin = 0;
let maxId = 0;
let mostFrequentGuardId = 0;
let mostSleep = 0;
let mostFrequentMinute = -1
Object.keys(schedule).forEach(guardId => {
    let minutes = 0;
    schedule[guardId].forEach(( sleepMin,minute ) => {
        if (sleepMin > 0) {
            // part 1- sum minutes
            minutes += sleepMin;
            // part 2: find minute with most sleep:
            if (sleepMin > mostSleep) {
                mostSleep = sleepMin;
...
console.log(`Day 3: Guard #${maxId} was asleep the most: ${maxMin} minutes.`);

// Step 4: Which minute is the most sleepy of that guard?
let maxSleep = 0;
maxMin = -1;
schedule[maxId].forEach(( sleepMins,minute ) => {
...
console.log(`Day 3: Guard #${maxId} was asleep the most (${maxSleep} minutes) at minute ${maxMin}.`);
console.log(`Day 3: Solution 1: (guard-id x minute): ${Number(maxId)*Number(maxMin)}`);

// ------------------- Part 2 -----------------------
// most frequent minute
console.log(`Day 3: Solution 2: Guard #${mostFrequentGuardId} slept most (${mostSleep} minutes) on minute: ${mostFrequentMinute}`);
console.log(`Day 3: Solution 2: (guard-id x minute): ${Number(mostFrequentGuardId)*mostFrequentMinute}`);
